...
func StartServer() {
	...
	api := r.Group("/api")
	{
		// Пользователи (заглушки для Postman)
		api.POST("/users/register", handler.ApiUserRegister)
		api.POST("/users/login", handler.ApiUserLogin)
		api.POST("/users/logout", handler.ApiUserLogout)
		api.GET("/users/me", handler.ApiUserMe)
		api.PUT("/users/me", handler.ApiUserUpdateMe)

		// Корзина/черновик
		api.GET("/records/draft/icon", handler.ApiGetDraftIcon)

		// Справочник стадий (услуг)
		api.GET("/stages", handler.ApiListStages)
		api.GET("/stages/:id", handler.ApiGetStage)
		api.POST("/stages", handler.ApiCreateStage)
		api.PUT("/stages/:id", handler.ApiUpdateStage)
		api.DELETE("/stages/:id", handler.ApiDeleteStage)
		api.POST("/stages/:id/image", handler.ApiSetStageImage)
		api.POST("/stages/:id/draft-add", handler.ApiAddStageToDraft)

		// Заявка (record)
		api.POST("/records/draft/items", handler.ApiAddDraftItem)
		api.PUT("/records/:id/items", handler.ApiUpdateRecordItem)
		api.DELETE("/records/:id/items", handler.ApiDeleteRecordItem)

		api.GET("/records", handler.ApiListRecords)
		api.GET("/records/:id", handler.ApiGetRecord)
		api.PUT("/records/:id", handler.ApiUpdateRecord)
		api.PUT("/records/:id/submit", handler.ApiSubmitRecord)
		api.PUT("/records/:id/resolve", handler.ApiResolveRecord)
		api.DELETE("/records/:id", handler.ApiDeleteRecord)
	}
